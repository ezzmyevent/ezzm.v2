...
<div class="container">
    <div class="d-flex justify-content-between mb-3 flex-wrap">
        <div class="d-flex">
            <h2 class="heading mb-0">Update Template</h2>
        </div>
        <div class="save-next">
    ...
    <div class="row">
        <div class="col-md-8">
            <form id="updatetemplate" method="post" enctype="multipart/form-data" action="{{route('tickets-save')}}">
                @csrf
                <input type="hidden" value="{{$id}}" id="updateid" name="updateid">
                <div class="row">
                    <div class="col-md-12">
                        <label class="label">Title <span class="error-required">*</span></label>
                        <input type="text" name="title" id="title" class="form-control" placeholder="" value="{{($templateDetail->title)?$templateDetail->title:''}}" required>
                    </div>
                </div>    
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <input type="text" name="subject" id="subject" class="form-control" placeholder="" value="{{($templateDetail->subject)?$templateDetail->subject:''}}">
                    </div>
                </div>  
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <input type="text" name="attachment" id="attachment" class="form-control" placeholder="" value="{{($templateDetail->attachment)?$templateDetail->attachment:''}}">
                    </div>
                </div>  
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <textarea class="form-control" id="content" name="content">{{($templateDetail->content)?$templateDetail->content:''}}</textarea>
                    </div>
                </div>   
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <input type="number" min="0" name="reminder_status" id="reminder_status" class="form-control" placeholder="" value="{{($templateDetail->reminder_status)?$templateDetail->reminder_status:''}}">
                    </div>
                </div>  
                    ...
                    <div class="col-md-12">
                        ...
                        <select class="form-control" id="user_category" name="user_category">
                                ...
                                @foreach ($categorieslist as $row)
                                    <option value="{{$row->category}}" {{($templateDetail->user_category == $row->category)?'selected':''}}>{{$row->category}}</option>
                                @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <select class="form-control" id="status" name="status">
                            ...
                            <option value="1" {{($templateDetail->status == '1')?'selected':''}}>Active</option>
                            <option value="0" {{($templateDetail->status == '0')?'selected':''}}>Deactive</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-12">
                        <button type="button" class="btn btn-save-next submit-btn">Update</button>
                    </div>
                </div>
...
$(document).on('click','.submit-btn', function (e) {
    ...
    var form = $('#updatetemplate')[0];
    var formData = new FormData(form);
    formData.append('content', editorContent);
	
    $.ajax({
        type: 'POST',                
        url: "{{ route('mail-template.update') }}",
        data: formData,
        contentType: false,
        processData: false,
        beforeSend: function() {
        ...
        success: function(response) {
            ...
            if ($.isEmptyObject(response.error)) {
                toastr.success('Mail Template successfully updated!');
                location.reload();
            } else {
                console.log(response.error);
                $.each(response.error, function(key, value) {
        ...
        error: function(xhr) {
            ...
            if (xhr.status === 400) {
                ...
                toastr.error('There was an issue updating the mail template. Please try again later.');
            }
        }
    });
